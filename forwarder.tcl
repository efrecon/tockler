...
set prg_args {
    ...
    -output    stdout      "Which output of the docker components to listen to"
}
...
proc ::result { component url line tok } {
    global FWD
...
# Callback whenever an output line from a docker component has been
# captured, generate the appropriate HTTP operation.
proc ::forward { component url type line } {
    global FWD
	...
	if { [catch {regexp -- $URLmatcher $url -> \
	    ...
	    if { $user ne "" } {
		set headers [list Authorization "Basic [binary encode base64 $user]"]
	    } else {
		set headers {}
	    }
		...
		if { [catch {::http::geturl $url \
				 ...
				 -command [list ::result $component $url $line]} tok] == 0 } {
...
foreach { component url } $FWD(-mapper) {
    $FWD(docker) attach $component [list ::forward $component $url] \
